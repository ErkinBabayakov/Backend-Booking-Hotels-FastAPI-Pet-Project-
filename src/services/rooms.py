...
from src.schemas.rooms import Room, RoomAddRequest, RoomAdd, RoomPatchRequest, RoomPatch
...
class RoomService(BaseService):
    ...
    async def get_room(self, hotel_id: int, room_id: int) -> Room:
        return await self.db.rooms.get_one_or_none_with_rels(hotel_id=hotel_id, id=room_id)
    ...
    async def create_room(self, hotel_id: int, room_data: RoomAddRequest):
        ...
        await self.db.room_facility.add_bulk(room_facilities_data)
    ...
    async def update_room(self, hotel_id: int, room_id: int, room_data: RoomAddRequest):
        ...
        await self.db.room_facility.set_room_facilities(room_id, facilities_ids=room_data.facilities_ids)
        await self.db.commit()

    async def partial_update_room(self, hotel_id: int, room_id: int, room_data: RoomPatchRequest):
        await HotelService(self.db).get_hotel_with_check(hotel_id)
        await self.get_room_with_check(room_id)
        ...
        if "facilities_ids" in _room_data_dict:
            await self.db.room_facility.set_room_facilities(
